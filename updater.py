...
def actualizar_calendario_y_jsons():
        ...
        for col in soup.find_all('div', class_='columna'):
            ...
            for fila in tabla.find_all('tr'):
                ...
                if a_p and len(a_eq) >= 2:
                    ...
                    datos.append({
                        "MATCHID": match_id, "ROUND": jornada, "SCORE_STR": resultado
                    })
    ...
    for _, partido in jugados.iterrows():
        ...
        if not os.path.exists(box_path) or not os.path.exists(pbp_path):
            print(f"   ⬇️ Descargando JSONs crudos para el partido {match_id}...")
            ...
            try:
                ...
                session.headers.update({"Authorization": f"Bearer {token}", "Origin": BASE_URL, "Referer": BASE_URL+"/", "Accept": "application/json"})
    ...
    print(f"✅ Descarga finalizada. {nuevos_descargados} partidos nuevos almacenados en raw_data.")

# ==============================================================================
# FASE 2: MOTOR MATEMÁTICO ETL (MÓDULO 3 DEL TFM)
# ==============================================================================
def to_float(val):
    try:
...
def procesar_estadisticas_acumuladas():
    ...
    # Creamos un ROSTER vacío si no existe para que no pete en el primer arranque
    ...
    print(f"📊 Partidos en raw_data: {len(partidos_totales)} | Ya procesados: {len(procesados_previos)} | Nuevos a procesar: {len(partidos_a_procesar)}")
    ...
    for match_id in partidos_a_procesar:
        try:
            ...
            # A) BOXSCORE Y ESTADÍSTICAS INDIVIDUALES
            match_players = []
            for t in teams:
            ...
            # C) PLAY BY PLAY Y LINEUPS SEGUROS
            if os.path.exists(pbp_path):
                ...
                # D) MOTOR DE LINEUPS Y DIFERENCIAL
    ...
    # E) APPEND ACUMULATIVO Y EXPORTACIÓN
...
if __name__ == "__main__":
    ...
    try:
        ...
        print(f"\n✅ ETL COMPLETADO. Partidos nuevos: {procesados} | Errores: {fails}")
    except Exception as e:
