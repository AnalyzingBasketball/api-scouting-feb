...
# ── CAMBIO 1: apunta al CSV que ya existe en el repo ──────────────────────────
...
def generar_html_boxscore(ruta_box_clean, ruta_pbp_clean, match_id, equipo_local, equipo_visit, fecha_partido):
    ...
    team_mapping_box = {
        equipo_local: actual_local_box,
        equipo_visit: actual_visit_box
    }
...
def cargar_datos_m13():
    global map_role_m13, map_pos_m13, map_name_m13, map_efg_m13, map_ts_m13, map_tov_m13, map_orb_m13, map_ftr_m13, map_usg_m13, custom_photos_m13, dicc_logos_m13
    map_role_m13.clear(); map_pos_m13.clear(); map_name_m13.clear()
    try:
        if os.path.exists(FILE_ROLES):
            df_roles = pd.read_csv(FILE_ROLES)
            df_roles['TEAM'] = df_roles.get('TEAM', pd.Series()).replace({'CLUB OURENSE BALONCESTO': 'CLOUD.GAL OURENSE BALONCESTO', 'OURENSE BALONCESTO': 'CLOUD.GAL OURENSE BALONCESTO'})
            for _, r in df_roles.iterrows():
                pid = safe_id(str(r.get('PLAYER_ID', '')))
...
def generar_html_splits(s_rnd, e_rnd, eq, m_filt):
    ...
    df_lineups_master['TEAM'] = df_lineups_master.get('TEAM', pd.Series()).replace({'CLUB OURENSE BALONCESTO': 'CLOUD.GAL OURENSE BALONCESTO', 'OURENSE BALONCESTO': 'CLOUD.GAL OURENSE BALONCESTO'})
    ...
    if df_split.empty: raise HTTPException(status_code=404, detail=f"No hay datos para el equipo {eq} en las jornadas {s_rnd} a {e_rnd}.")
    ...
    if arch_stats.empty: raise HTTPException(status_code=404, detail=f"Ningún quinteto superó el filtro de {m_filt} minutos jugados juntos.")
    ...
    <style>
        ...
        .header-title-block h1 {{ margin: 0; font-size: 32px; color: #1a202c; text-transform: uppercase; }}
        ...
        .team-title-block h2 {{ margin: 0; font-size: 28px; color: #2d3748; text-transform: uppercase; font-weight: 800; }}
        ...
        <div class="top-banner">
            <div class="top-logos"><img src="data:image/png;base64,{logo_feb_b64}" class="logo-side"><img src="data:image/png;base64,{logo_empresa_b64}" class="logo-center"><img src="data:image/png;base64,{logo_liga_b64}" class="logo-side"></div>
            <div class="header-title-block">
                <h1>Tactical Splits Slicer</h1>
                <div class="subtitle">Primera FEB | {round_title} | Filter: Min {m_filt} minutes played</div>
            </div>
        </div>
    ...
    out_name = f"SPLIT_J{s_rnd}_J{e_rnd}_{eq_f}.html"
...
def cargar_datos_m14():
    global map_role_m14, map_pos_m14, map_name_m14, map_efg_m14, map_ts_m14, map_tov_m14, map_orb_m14, map_ftr_m14, map_usg_m14, custom_photos_m14, dicc_logos_m14
...
# ── CAMBIO 2 y 3: HTML_BOXSCORE_AGREGADO_M14 reescrita con fixes de fotos y KeyError ──
def HTML_BOXSCORE_AGREGADO_M14(df_all_box, eq_objetivo, context_str, team_games_count):
    ...
    for team in [eq_objetivo, "OPPONENTS"]:
        ...
        agg_funcs = {
            'GP': 'sum', 'Starter': 'sum', 'Min_Sec_Num': 'sum',
            'PTS': 'sum', 'PIR': 'sum',
            '2PM': 'sum', '2PA': 'sum', '3PM': 'sum', '3PA': 'sum',
            'FTM': 'sum', 'FTA': 'sum',
        ...
        t_df_grouped['sort_idx'] = t_df_grouped['Player_ID'].apply(
            lambda pid: get_classic_order_m14(safe_id(str(pid)))
        )
    ...
    tm_FGA = t_tot['2PA'] + t_tot['3PA']
    tm_FGM = t_tot['2PM'] + t_tot['3PM']
    opp_FGA = opp_tot['2PA'] + opp_tot['3PA']
    tm_Poss = tm_FGA + 0.44 * t_tot['FTA'] + t_tot['TOV']
    opp_Poss = opp_FGA + 0.44 * opp_tot['FTA'] + opp_tot['TOV']

    html_tables += f"""<h2 class="team-section-title">{team}</h2>
    <div class="table-container"><table>
    <thead class="group-headers"><tr>
        <th colspan="5" class="bg-info">INFO</th>
        <th colspan="13" class="bg-trad">TRADITIONAL (PER GAME)</th>
        <th colspan="9" class="bg-shoot">SHOOTING (PER GAME)</th>
        <th colspan="15" class="bg-adv">ADVANCED METRICS (AGGREGATED)</th>
    </tr></thead>
    ...
    for p in t_data['players']:
        ...
        # ── FIX 3A: fotos desde custom_photos_m14, fallback a URL de la FEB ──
        ...
        par3 = (p['3PA'] / fga * 100) if fga > 0 else 0
        ftr = (p['FTA'] / fga * 100) if fga > 0 else 0
        ...
        gmsc = (p['PTS'] + 0.4 * fgm - 0.7 * fga - 0.4 * (p['FTA'] - p['FTM'])
                + 0.7 * p['OREB'] + 0.3 * p['DREB'] + p['STL']
                + 0.7 * p['AST'] + 0.7 * p['BLK'] - 0.4 * p['PF'] - p['TOV'])
        tot_gmsc += gmsc
        ...
        html_tables += (
            f"<tr>"
            f"<td class='td-info'><img src='{foto}' class='player-photo'></td>"
            f"<td class='td-info player-name'>{player}</td>"
            f"<td class='td-info font-bold text-blue' style='font-size:11px;'>{role}</td>"
            f"<td class='td-info font-bold text-gray'>{gp}</td>"
            f"<td class='td-info font-bold text-blue'>{gs}</td>"
            ...
            f"<td class='td-trad'>{p['OREB']/gp:.1f}</td>"
            f"<td class='td-trad'>{p['DREB']/gp:.1f}</td>"
            f"<td class='td-trad font-bold'>{p['TREB']/gp:.1f}</td>"
            f"<td class='td-trad'>{p['AST']/gp:.1f}</td>"
            f"<td class='td-trad text-green'>{p['STL']/gp:.1f}</td>"
            f"<td class='td-trad text-red'>{p['TOV']/gp:.1f}</td>"
            f"<td class='td-trad'>{p['BLK']/gp:.1f}</td>"
            f"<td class='td-trad text-gray'>{p['FD']/gp:.1f}</td>"
            f"<td class='td-trad text-gray'>{p['PF']/gp:.1f}</td>"
            f"<td class='td-trad font-bold {pm_class}'>{pm_str}</td>"
            f"<td class='td-shoot font-bold'>{p['2PM']/gp:.1f}</td>"
            f"<td class='td-shoot text-gray'>{p['2PA']/gp:.1f}</td>"
            f"<td class='td-shoot'>{fg2_pct:.0f}%</td>"
            f"<td class='td-shoot font-bold'>{p['3PM']/gp:.1f}</td>"
            f"<td class='td-shoot text-gray'>{p['3PA']/gp:.1f}</td>"
            f"<td class='td-shoot'>{fg3_pct:.0f}%</td>"
            f"<td class='td-shoot font-bold'>{p['FTM']/gp:.1f}</td>"
            f"<td class='td-shoot text-gray'>{p['FTA']/gp:.1f}</td>"
            f"<td class='td-shoot'>{ft_pct:.0f}%</td>"
            f"<td class='td-adv font-bold'>{gmsc/gp:.1f}</td>"
            f"<td class='td-adv'>{ts_pct:.1f}%</td>"
            f"<td class='td-adv'>{efg_pct:.1f}%</td>"
            f"<td class='td-adv text-gray'>{par3:.1f}%</td>"
            f"<td class='td-adv text-gray'>{ftr:.1f}%</td>"
            f"<td class='td-adv font-bold text-blue'>{usg_pct:.1f}%</td>"
            f"<td class='td-adv'>{orb_pct:.1f}%</td>"
            f"<td class='td-adv'>{drb_pct:.1f}%</td>"
            f"<td class='td-adv text-gray'>{trb_pct:.1f}%</td>"
            f"<td class='td-adv'>{ast_pct:.1f}%</td>"
            f"<td class='td-adv'>{stl_pct:.1f}%</td>"
            f"<td class='td-adv'>{blk_pct:.1f}%</td>"
            f"<td class='td-adv'>{tov_pct:.1f}%</td>"
            f"<td class='td-adv font-bold'>{ppp:.2f}</td>"
            f"<td class='td-adv font-bold'>{pps:.2f}</td>"
            f"</tr>"
        )

    # ── FIX 3B: claves corregidas OREB/DREB/TREB en la fila de totales ──
    ...
    html_tables += (
        ...
        f"<td colspan='5' class='td-info' style='text-align:right; padding-right:15px;'><b>TEAM AVERAGES</b></td>"
    ...
    <style>
        ...
        .table-container {{ background: #fff; border-radius: 12px; overflow-x: auto; margin-bottom: 25px; }}
        table {{ width: 100%; border-collapse: collapse; text-align: center; white-space: nowrap; }}
        .bg-info {{ background: #2d3748 !important; color: #fff !important; }}
        .bg-trad {{ background: #4a5568 !important; color: #fff !important; }}
        .bg-shoot {{ background: #2b6cb0 !important; color: #fff !important; }}
        .bg-adv {{ background: #2c7a7b !important; color: #fff !important; }}
        .col-headers th {{ background: #edf2f7; color: #4a5568; font-size: 11px; padding: 10px 5px; border-bottom: 2px solid #cbd5e0; }}
        td {{ padding: 8px 4px; font-size: 12px; border-bottom: 1px solid #edf2f7; vertical-align: middle; }}
        .td-info {{ background: #ffffff; }} .td-trad {{ background: #f8fafc; }} .td-shoot {{ background: #ebf8fa; }} .td-adv {{ background: #f0fff4; }}
        .total-row td {{ background: #e2e8f0; font-weight: bold; border-top: 2px solid #a0aec0; }}
        .player-name {{ text-align: left; font-weight: 700; color: #2d3748; font-size: 13px; }}
        .player-photo {{ width: 36px; height: 36px; border-radius: 50%; border: 2px solid #cbd5e0; object-fit: cover; }}
        ...
        <div class="header-container">
            <div class="top-logos">
                <img src="data:image/png;base64,{logo_feb_b64}" class="logo-side">
                <img src="data:image/png;base64,{logo_empresa_b64}" class="logo-center">
                <img src="data:image/png;base64,{logo_liga_b64}" class="logo-side">
            </div>
            <div class="match-info">
                <div class="team-score-block"><img src="{logo_url}" class="team-shield"><h1>{eq_objetivo}</h1></div>
                <div class="scores">AGGREGATED BOXSCORE (PER GAME)<br>
                <span style="font-weight:600; font-size:17px; color:#718096;">{context_str}</span></div>
            </div>
        </div>
        {html_tables}
        <div class="legend-grid">
            <div class="legend-item"><b>PIC / PLAYER:</b> Player Info.<br><b>GP:</b> Games Played.<br><b>GS:</b> Games Started.<br><b>MIN:</b> Minutes Played.<br><b>PTS:</b> Points Scored.<br><b>PIR:</b> Performance Index Rating.<br><b>+/-:</b> Plus/Minus point differential.</div>
            <div class="legend-item"><b>ORB:</b> Offensive Rebounds.<br><b>DRB:</b> Defensive Rebounds.<br><b>TRB:</b> Total Rebounds.<br><b>AST:</b> Assists.<br><b>STL:</b> Steals.<br><b>TOV:</b> Turnovers.</div>
            <div class="legend-item"><b>BLK:</b> Blocks.<br><b>PFD:</b> Personal Fouls Drawn.<br><b>PF:</b> Personal Fouls Committed.<br><b>2PM/A:</b> 2-Point Goals Made/Attempted.<br><b>3PM/A:</b> 3-Point Goals Made/Attempted.<br><b>FTM/A:</b> Free Throws Made/Attempted.</div>
            <div class="legend-item"><b>GmSc:</b> Game Score (Productivity metric).<br><b>TS%:</b> True Shooting Percentage.<br><b>eFG%:</b> Effective Field Goal Percentage.<br><b>3PAr:</b> 3-Point Attempt Rate.<br><b>FTr:</b> Free Throw Attempt Rate.<br><b>USG%:</b> Usage Percentage.</div>
            <div class="legend-item"><b>ORB% / DRB% / TRB%:</b> Rebound Percentages.<br><b>AST%:</b> Assist Percentage.<br><b>STL% / BLK%:</b> Steal &amp; Block Percentages.<br><b>TOV%:</b> Turnover Percentage.<br><b>PPP:</b> Points Per Possession.<br><b>PPS:</b> Points Per Shot.</div>
        </div>
        <div class="footer">© 2026 Analizing Basketball | <a href='https://www.analizingbasketball.com' target='_blank'>www.analizingbasketball.com</a></div>
    </body></html>
    """
...
# ==============================================================================
# 7. INTERFAZ API REST PRINCIPAL (ENDPOINTS)
# ==============================================================================
app = FastAPI()
...
def generar_scouting(jornada: int = 22, equipo: str = "MOVISTAR ESTUDIANTES", tipo_reporte: str = "quintetos"):
    ...
    if not extraer_partido_api(p['match_id']):
        raise HTTPException(status_code=500, detail="Error al descargar datos en vivo.")
...
def splits_api(s_rnd: int = 1, e_rnd: int = 22, eq: str = "MOVISTAR ESTUDIANTES", m_filt: int = 10):
    if s_rnd > e_rnd:
        raise HTTPException(status_code=400, detail="La jornada de inicio no puede ser posterior a la jornada final.")
...
def generar_contextual(eq: str = "MOVISTAR ESTUDIANTES", venue: str = "ALL", n_games: int = 3, m_filt: int = 10, tipo_reporte: str = "quintetos"):
    ...
    # ── RAMA QUINTETOS: intacta ────────────────────────────────────────────────
    if tipo_reporte.lower() == "quintetos":
        if not os.path.exists(FILE_CALENDAR):
            construir_calendario_maestro()
        if not os.path.exists(FILE_CALENDAR):
            raise HTTPException(status_code=500, detail="Error crítico: No se pudo generar el archivo de calendario maestro.")

        df_cal = pd.read_csv(FILE_CALENDAR)
        eq_clean = clear_string(eq).replace("CLOUD.GAL ", "").replace("GRUPO CAESA SEGUROS ", "").replace(" ", "")

        df_cal_jugados = df_cal[df_cal['resultado'].astype(str).str.contains(r'\d+\s*-\s*\d+', regex=True, na=False)].copy()
        if df_cal_jugados.empty:
            raise HTTPException(status_code=404, detail="El calendario indica que no hay partidos jugados en toda la liga.")

        df_cal_jugados['JORNADA_NUM'] = df_cal_jugados['jornada'].astype(str).str.extract(r'(\d+)')[0].astype(float)

        mask_local = df_cal_jugados['equipo_local'].apply(lambda x: eq_clean in clear_string(str(x)).replace(" ", ""))
        mask_visit = df_cal_jugados['equipo_visitante'].apply(lambda x: eq_clean in clear_string(str(x)).replace(" ", ""))

        if venue == "HOME": df_team_games = df_cal_jugados[mask_local].copy()
        elif venue == "AWAY": df_team_games = df_cal_jugados[mask_visit].copy()
        else: df_team_games = df_cal_jugados[mask_local | mask_visit].copy()

        if df_team_games.empty:
            raise HTTPException(status_code=404, detail=f"No se encontraron partidos para {eq} en condición {venue}.")

        df_team_games = df_team_games.sort_values(by='JORNADA_NUM', ascending=False).head(n_games)
        jornadas_validas = df_team_games['JORNADA_NUM'].dropna().astype(int).tolist()
        ...
        eq_f = eq.replace(" ", "_")
        out_name = f"CONTEXT_LINEUPS_{eq_f}_{len(jornadas_validas)}_{venue}.html"
        ruta_final = os.path.join(REPORTS_DIR, out_name)
        with open(ruta_final, "w", encoding="utf-8") as f:
            f.write(html_content)
        return HTMLResponse(content=html_content, status_code=200)

    # ── RAMA BOXSCORE: reescrita con Cambio 2 ─────────────────────────────────
    else:
        ...
        df_master['TEAM'] = df_master['TEAM'].replace(TEAM_FIXES_BOX)

        # Obtener partidos del equipo directamente desde el boxscore (sin calendario)
        ...
        # Renombrado limpio y específico basado en columnas reales del CSV
        rename_dict = {
            'TEAM':       'Team',
            'IS_STARTER': 'Starter',
            'PLAYER_ID':  'Player_ID',
            'PLAYER_NAME':'Player',
            'MIN_SECS':   'Min_Sec_Num',   # ya en segundos, NO aplicar parse_min
            'FGM_2':      '2PM',
            'FGA_2':      '2PA',
            'FGM_3':      '3PM',
            'FGA_3':      '3PA',
        ...
        # Marcar equipo objetivo vs rivales
        ...
        eq_f = eq.replace(" ", "_")
        out_name = f"CONTEXT_BOXSCORE_{eq_f}_{jornadas_count}_{venue}.html"
        ruta_final = os.path.join(REPORTS_DIR, out_name)
        with open(ruta_final, "w", encoding="utf-8") as f:
            f.write(html_content)
