...
DATA_DIR = os.path.join(BASE_DIR, "data")  # Confirmado que en GitHub es "data"
...
def cargar_roles_m12():
    """Carga los roles en tiempo real para el Módulo 12"""
...
def cargar_datos_m13():
    """Carga los roles y metadatos en tiempo real para el Módulo 13"""
...
def create_signatures_m13(row):
    """Mapea los IDs de los 5 jugadores a sus roles K-Means"""
...
def limpiar_boxscore_api_m14(match_id):
    # NOTA: Esta función se mantiene intacta pero ya no se usará en el endpoint de Boxscore.
    # El Boxscore del M14 ahora procesa de manera segura el Archivo Maestro directamente.
    df_box = pd.read_csv(os.path.join(DATA_DIR, f"boxscore_{match_id}.csv"))
    mapeo = {'team_name': 'Team', 'no': 'No', 'inn': 'Starter', 'name': 'Player', 'minFormatted': 'Min', 'pts': 'PTS',
...
def HTML_BOXSCORE_AGREGADO_M14(df_all_box, eq_objetivo, context_str, team_games_count):
    ...
    for team in [eq_objetivo, "OPPONENTS"]:
        ...
        t_df_grouped['sort_idx'] = t_df_grouped['Player_ID'].apply(get_classic_order_m14)
    ...
    for p in t_data['players']:
        ...
        p_data = custom_photos_m14.get(pid, {})
        full_name = p_data.get("PLAYER_NAME", map_name_m14.get(pid, p['Player']))
        player = get_short_name(full_name)
        role = map_role_m14.get(pid, "N/A"); foto = p.get('Logo_URL'); foto = "https://via.placeholder.com/40/cbd5e0/ffffff?text=+" if pd.isna(foto) else foto
...
def generar_contextual(eq: str = "MOVISTAR ESTUDIANTES", venue: str = "ALL", n_games: int = 3, m_filt: int = 10, tipo_reporte: str = "quintetos"):
    ...
    # Usamos lectura defensiva del calendario para soportar nombres pre y post-automatización
    FILE_CALENDAR_ACTUAL = os.path.join(DATA_DIR, "CALENDAR_PRIMERAFEB_2526.csv")
    if os.path.exists(FILE_CALENDAR_ACTUAL):
        df_cal = pd.read_csv(FILE_CALENDAR_ACTUAL)
    else:
        if not os.path.exists(FILE_CALENDAR):
            construir_calendario_maestro()
        if not os.path.exists(FILE_CALENDAR):
    ...
    # Estandarización de columnas vengan del bot o del scraper manual
    col_resultado = 'RESULTADO' if 'RESULTADO' in df_cal.columns else 'resultado'
    col_jornada = 'ROUND' if 'ROUND' in df_cal.columns else 'jornada'
    col_local = 'LOCAL' if 'LOCAL' in df_cal.columns else 'equipo_local'
    col_visit = 'AWAY' if 'AWAY' in df_cal.columns else 'equipo_visitante'
    col_match = 'MATCH_ID' if 'MATCH_ID' in df_cal.columns else 'match_id'
    
    eq_clean = clear_string(eq).replace("CLOUD.GAL ", "").replace("GRUPO CAESA SEGUROS ", "").replace(" ", "")
    
    df_cal_jugados = df_cal[df_cal[col_resultado].astype(str).str.contains(r'\d+\s*-\s*\d+', regex=True, na=False)].copy()
    ...
    df_cal_jugados['JORNADA_NUM'] = df_cal_jugados[col_jornada].astype(str).str.extract(r'(\d+)')[0].astype(float)
    
    mask_local = df_cal_jugados[col_local].apply(lambda x: eq_clean in clear_string(str(x)).replace(" ", ""))
    mask_visit = df_cal_jugados[col_visit].apply(lambda x: eq_clean in clear_string(str(x)).replace(" ", ""))
    ...
    match_ids = df_team_games[col_match].astype(str).tolist()
    ...
    if tipo_reporte.lower() == "quintetos":
        ...
        if not os.path.exists(FILE_MASTER_BOXSCORE):
            raise HTTPException(status_code=404, detail="Archivo maestro BOXSCORE_PRIMERAFEB_2526.csv no encontrado en el servidor. La agregación depende de este fichero.")
            
        df_master_box = pd.read_csv(FILE_MASTER_BOXSCORE)
        
        # 1. Filtro seguro de jornadas o match_ids
        ...
        if col_master_round:
            ...
            col_master_match = 'MATCH_ID' if 'MATCH_ID' in df_master_box.columns else ('match_id' if 'match_id' in df_master_box.columns else None)
            if col_master_match:
        ...
        if df_filtered.empty:
            raise HTTPException(status_code=404, detail="No se encontraron datos en el Boxscore Maestro para las jornadas de este equipo.")
            
        # 2. Diccionario de Mapeo Universal (Soporta nombres del bot y formato Raw API)
        mapeo = {
            'TEAM': 'Team', 'team_name': 'Team', 'Team': 'Team',
            'no': 'No', 'NO': 'No',
            'inn': 'Starter', 'STARTER': 'Starter', 'Starter': 'Starter',
            'name': 'Player', 'PLAYER': 'Player', 'PLAYER_NAME': 'Player', 'Player': 'Player',
            'minFormatted': 'Min', 'MINUTES': 'Min', 'min': 'Min', 'Min': 'Min',
            'pts': 'PTS', 'PTS': 'PTS',
            'p2m': '2PM', '2PM': '2PM',
            'p2a': '2PA', '2PA': '2PA',
            'p3m': '3PM', '3PM': '3PM',
            ...
            'pllss': '+/-', '+/-': '+/-', 'PLUS_MINUS': '+/-',
            'val': 'PIR', 'PIR': 'PIR',
            'id': 'Player_ID', 'PLAYER_ID': 'Player_ID', 'Player_ID': 'Player_ID',
            'logo': 'Logo_URL', 'LOGO_URL': 'Logo_URL', 'Logo_URL': 'Logo_URL'
        }
        
        # Renombrado condicional sin perder columnas críticas
        rename_dict = {}
        ...
        # Eliminamos duplicados si los hubiese (ej: team_name y TEAM juntos mapean a Team)
        df_b_clean = df_b_clean.loc[:,~df_b_clean.columns.duplicated()].copy()
        
        # 3. Limpieza y Casteo Fuerte
        if 'Starter' in df_b_clean.columns:
            df_b_clean['Starter'] = df_b_clean['Starter'].astype(str).str.strip().str.lower().apply(lambda x: 1 if x in ['1', 'true', 'yes', '*'] else 0)
        else:
        ...
        # 4. Asignación Segura del Equipo vs Oponentes con la función heredada del M12
        ...
        # 5. Volcado al Reporte HTML Final
